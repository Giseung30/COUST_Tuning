...
$query = mysqli_query($conn, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Unity_CarDictionary'"); //컬럼명 저장
while($row = mysqli_fetch_assoc($query)) //결과가 존재하면
  array_push($columns, $row["COLUMN_NAME"]); //컬럼명 배열에 추가

$query = mysqli_query($conn, "Select * From Unity_CarDictionary"); //쿼리 결과 저장
$result = array(); //결과 배열
while($row = mysqli_fetch_assoc($query)) //결과가 존재하면
{
  $rowArray = array(); //행 배열
